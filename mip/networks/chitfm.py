...
def build_slot_noise_embedding(
    slot_noise_levels: torch.Tensor,
    d_model: int,
):
    """
    Fixed sinusoidal embedding for per-slot noise levels.
    ...
    Returns:
        emb: (B, T, d_model)
    """
    if slot_noise_levels.ndim == 1:
        slot_noise_levels = slot_noise_levels.unsqueeze(0)  # (1, T)

    B, T = slot_noise_levels.shape
    device = slot_noise_levels.device
    dtype = slot_noise_levels.dtype

    assert d_model % 2 == 0, "d_model must be even for sinusoidal noise embedding"

    half_dim = d_model // 2
    freq = torch.exp(
        -math.log(10000.0) * torch.arange(half_dim, device=device, dtype=dtype) / max(half_dim - 1, 1)
    )  # (half_dim,)

    angles = slot_noise_levels.unsqueeze(-1) * freq.view(1, 1, half_dim)  # (B, T, half_dim)
...
class ChiTransformer(BaseNetwork):
    ...
    def forward(
        ...
        slot_noise_levels: torch.Tensor | None = None,
    ):
        ...
        # Add fixed per-slot noise embedding if provided
        if slot_noise_levels is not None:
            noise_emb = build_slot_noise_embedding(
                slot_noise_levels=slot_noise_levels,
                d_model=self.d_model,
            )  # (b, Ta, d_model) or (1, Ta, d_model)
            if noise_emb.shape[0] == 1:
                noise_emb = noise_emb.expand(b, -1, -1)
            input_emb = input_emb + noise_emb
