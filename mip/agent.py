...
class TrainingAgent:
    ...
    def _create_update_impl(self):
        ...
        def update_impl(data: TensorDict):
            ...
            # Optional per-loss scalars (e.g. loss_coarse) are passed through for logging.
            td_dict = {
                "loss": loss.detach(),
                "grad_norm": grad_norm.detach(),
            }
            for k, v in loss_aux.items():
                td_dict[k] = torch.as_tensor(
                    v, device=loss.device, dtype=torch.float32
                )
            result = TensorDict(td_dict, batch_size=())
            return result
    ...
    def update(
        ...
        Returns:
            Dictionary with at least ``loss`` and ``grad_norm``; may include extra
            float scalars from the active loss (e.g. ``loss_coarse``) as tensors.
        """
