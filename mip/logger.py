...
class Logger:
    ...
    def log(self, d, category):
        ...
        # For JSON logging, create a copy without wandb.Image objects AND convert numpy types
        json_safe_dict = {"step": int(d["step"]) if isinstance(d["step"], (np.integer, np.floating)) else d["step"]}
        for k, v in d.items():
            if not isinstance(v, self._wandb.Image):
